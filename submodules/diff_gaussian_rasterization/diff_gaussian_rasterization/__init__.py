...
import sys
import os
from diff_gaussian_rasterization_lang import _C as _C_lang

def cpu_deep_copy_tuple_spla(input_tuple):
    copied_tensors = [item.cpu().clone() if isinstance(item, torch.Tensor) else item for item in input_tuple]
    return tuple(copied_tensors)

def rasterize_gaussians_spla(
    means3D,
    means2D,
    sh,
    colors_precomp,
    ...
    return _RasterizeGaussians_spla.apply(
        means3D,
        means2D,
        sh,
        colors_precomp,
...
class _RasterizeGaussians_spla(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        means3D,
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple_spla(args) # Copy them before they can be corrupted
            try:
...
class GaussianRasterizationSettings_spla(NamedTuple):
    image_height: int
    image_width: int 
    tanfovx : float
    tanfovy : float
...
class GaussianRasterizer_spla(nn.Module):
    ...
    def forward(self, means3D, means2D, opacities, shs = None, colors_precomp = None, scales = None, rotations = None, cov3D_precomp = None, theta=None, rho=None):
        ...
        return rasterize_gaussians_spla(
            means3D,
            means2D,
            shs,
            colors_precomp,
